...
def train(args, model, train_features, dev_features, test_features):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                if step % args.gradient_accumulation_steps == 0:
                    ...
                    if global_step % log_step == 0:
                        ...
                        #         epoch, global_step, elapsed / 60, optimizer.state_dict()['param_groups'][0]['lr'], cur_loss * 1000))
                        logging(
                            '| epoch {:2d} | step {:4d} | min/b {:5.2f} | lr {} | train loss {:5.3f}'.format(
                ...
                if (step + 1) == len(train_dataloader) - 1 or (
                    ...
                    # logging(
                    # '| epoch {:3d} | time: {:5.2f}s | train_result:{}'.format(epoch, time.time() - eval_start_time,
                    #                                                           train_output))
                    logging(
    ...
    optimizer_grouped_parameters = [
        {"params": [p for n, p in cur_model.named_parameters() if any(nd in n for nd in bert_layer)],
         "lr": args.bert_lr},
